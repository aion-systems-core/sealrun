...
Define **repeatable** checks for governance bundle validity and **policy evaluation** behavior. This suite complements **RBAC** and **OIDC** controls: it validates the JSON policy consumed by `sealrun enterprise policy-api`, typically derived from the YAML bundles in `docs/governance/bundles/`.
...
4. Archive stdout and policy artifacts using `docs/templates/audit-evidence-governance-decision-template.md`.
